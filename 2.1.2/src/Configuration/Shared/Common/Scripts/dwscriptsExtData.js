...
function writeAttr(obj,relFile,attr) {
	DWfile.write(dw.getConfigurationPath() + "/" + relFile,dumpVar(obj),attr);
}
...
function dwscripts_applySB(paramObj, sbObj, sbName)
{
...
function dwscripts_applyDocEdits(maintainSelection)
{
  ...
  // commit all scheduled edits
  docEdits.apply(maintainSelection);
}
...
function docEdits_apply()
{
  ...
  try
  {
    ...
    if (docEdits.editList.length > 0)
    {
      ...
      {
        // DEBUG alert(docEdits.editList[i].toString());
      ...
      // DEBUG alert("The new document source is:\n\n" + newSrc.join(""));
  ...
  else if (this.weight.indexOf("nodeAttribute") == 0)
  {
    ...
    if (this.weightInfo && this.weightInfo.length)
    {
      ...
      if (!pos)  //if attrib doesn't exist
      {
        ...
        //change text to include entire attribute

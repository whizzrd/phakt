...
function applyServerBehavior(sbObj)
{
  ...
  if (!errStr)
  {
    ...
    if (!errStr)
    {
      paramObj["ConnectionPath"] = _ConnectionName.getPath();
...
//   E.g:
//
//   "UPDATE customers SET FirstName = " . GetSQLValueString($HTTP_POST_VARS['txtFirst'], "text") . ", LastName = " . GetSQLValueString($HTTP_POST_VARS['txtlast'], "text") . WHERE CustNo = " . GetSQLValueString($HTTP_POST_VARS['txtcustno'], "int");
//
// ARGUMENTS:
//   none
//
...
function SBUpdateRecord_postProcessFind()
{
  ...
  if (this.getParameter("ConnectionName") == "") {
